...
import { Nav } from "../Nav";
import "./Bike.css";
import Bikes from "../Dashboard-images/Dashboard-Bike.png";
...
export const Bike = () => {
  ...
  const [bikePickerName, setbikePickerName] = useState(null);
  ...
  const fromLocations = [...new Set(bikeRoutes.map((r) => r.fromLocation))].filter(Boolean);
  const toLocations = [...new Set(bikeRoutes.map((r) => r.toLocation))].filter(Boolean);
  ...
  const bikeRoutesSearch = () => {
      const filtered = bikeRoutes.filter((route) => {
  ...
  useEffect(() => {
    ...
    setFilteredRoutes(filtered);
  }, [searchFrom, searchTo, bikeRoutes]);
  ...
  useEffect(() => {
    fetch("https://randomuser.me/api/?results=10000&nat=IN")
      ...
      .then((json) => {
        ...
        setbikePickerName(users[randomIndex]);
      })
  ...
  return (
    <div className="bike-page">
      ...
      <div className="bike-booking">
        <div className="bike-booking-left">
          <h1>Request a Bike Ride</h1>
          <p>Up to 50% off your first 5 bike rides. T&Cs apply. *</p>
          ...
          <div className="bike-buttons">
            <button onClick={bikeRoutesSearch}>Check Prices</button>
          </div>
        ...
        <div className="bike-booking-right">
          <img src={Bikes} alt="Bike ride illustration" />
        </div>
      </div>

      <div className="bike-routes">
        <h1>Quick Bike Rides, Low Fares</h1>
        <h3>Top Boarding & Dropping Points in Chennai</h3>
        <p>
          Choose from popular bike routes across Chennai for a fast and affordable commute.
          Prices are estimated.
        </p>
        <div className="bike-routes-container">
          {bikeRouteList.map((region) => (
            <section className="bike-region" key={region.id}>
              <h2>{region.title}</h2>
              <ul>
                {region.routes.map((route, idx) => (
                  <li key={idx} onClick={() => handleSelectRoute(route)}>
                    <div className="bike-route-fromto">{route.fromTo}</div>
                    <div className="bike-route-price">{route.price}</div>
                  </li>
                ))}
              </ul>
            </section>
          ...
          <div className="qr-container" onClick={(e) => e.stopPropagation()}>
            {step === 1 && (
              <>
                ...
                <QRCodeCanvas
                  value={`upi://pay?pa=OneCitOnePay@oksbi&pn=Raja&tn=Bike Ride&am=${getNumericPrice(selectedRoute.price)}&cu=INR`}
                  size={180}
                />
                <p className="upi-info">
                  Pay {selectedRoute.price} to <strong>Raja</strong><br />
                  UPI ID: <strong>OneCitOnePay@oksbi</strong><br />
                  Purpose: <strong>Bike Ride</strong>
                </p>
              ...
              <>
                ...
                {bikePickerName ? (
                  <div className="pickuper-info">
                    <strong>Biker Name:</strong> {bikePickerName.fullName} <br />
                    <strong>Biker Phone:</strong> {bikePickerName.phone}<br />
                    <strong>OTP:</strong> {OTP}
                  </div>
                ) : (
                  <p>Loading random rider...</p>
                )}
                <div className="qr-popup-button">
