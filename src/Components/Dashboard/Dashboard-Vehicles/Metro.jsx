...
import { Nav } from "../Nav";
import "./Metro.css";
import metro1 from "../Dashboard-images/Dashboard-Metro.png";
...
export const Metro = () => {
  ...
  // const [metroPickerName, setMetroPickerName] = useState(null);

  // const OTP = Math.floor(Math.random() * 100000);
  const loggedInUser = JSON.parse(localStorage.getItem("loggedInUser")) || {};

  // Static default Metro route list
  const metroRouteList = [
    {
      id: "north-chennai",
      title: "North Chennai",
      routes: [
    ...
    },
    {
      ...
      routes: [
        ...
        { fromTo: "OMR → Sholinganallur", price: "₹40" },
  ...
  const fromLocations = [...new Set(metroRoutes.map((r) => r.fromLocation))].filter(Boolean);
  const toLocations = [...new Set(metroRoutes.map((r) => r.toLocation))].filter(Boolean);
  ...
  useEffect(() => {
    ...
    setFilteredRoutes(filtered);
  }, [searchFrom, searchTo, metroRoutes]);
  ...
  return (
    <div className="metro-page">
      ...
      <div className="metro-booking">
        <div className="metro-booking-left">
          <h1>Request a Metro Ride</h1>
          <p>Up to 50% off your first 5 metro rides. T&Cs apply. *</p>

          <div className="metro-inputs">
          ...
          <div className="metro-buttons">
            <button onClick={metroRoutesSearch}>Check Prices</button>
          </div>

          {showPopup && (
            <div className="popup-overlay" onClick={() => setShowPopup(false)}>
              <div className="popup-content" onClick={(e) => e.stopPropagation()}>
                <h2>Available Metro Routes</h2>
        ...
        <div className="metro-booking-right">
          <img src={metro1} alt="Metro ride illustration" />
        </div>
      </div>

      <div className="metro-routes">
        <h1>Quick Metro Rides, Low Fares</h1>
        <h3>Top Boarding & Dropping Metro Stations in Chennai</h3>
        <p>
          Choose from popular metro routes across Chennai for a fast and affordable commute.
          Prices are estimated.
        </p>
        <div className="metro-routes-container">
          {metroRouteList.map((region) => (
            <section className="metro-region" key={region.id}>
              <h2>{region.title}</h2>
              <ul>
                {region.routes.map((route, idx) => (
                  <li key={idx} onClick={() => handleSelectRoute(route)}>
                    <div className="metro-route-fromto">{route.fromTo}</div>
                    <div className="metro-route-price">{route.price}</div>
                  </li>
                ))}
              </ul>
            </section>
          ...
          <div className="qr-container" onClick={(e) => e.stopPropagation()}>
            {step === 1 && (
              <>
                ...
                <QRCodeCanvas
                  value={`upi://pay?pa=OneCitOnePay@oksbi&pn=Raja&tn=Metro Ride&am=${getNumericPrice(selectedRoute.price)}&cu=INR`}
                  size={180}
                />
                <p className="upi-info">
                  Pay {selectedRoute.price} to <strong>Raja</strong><br />
                  UPI ID: <strong>OneCitOnePay@oksbi</strong><br />
                  Purpose: <strong>Metro Ride</strong>
                </p>
              ...
              <>
                ...
                {/* {metroPickerName ? (
                  <div className="pickuper-info">
                    <strong>Metro Conductor:</strong> {metroPickerName.fullName} <br />
                    <strong>Phone:</strong> {metroPickerName.phone}<br />
                    <strong>OTP:</strong> {OTP}
                  </div>
                ) : (
                  <p>Loading random metro staff...</p>
                )} */}
                <div className="qr-popup-button">
